...
public class PdfGenerationService : IPdfGenerationService
{
    ...
    private void AddSide(PdfDocument pdf, Document document, PageSize pageSize, byte[] backgroundImage, TemplateLayerDto? layer)
    {
            ...
            foreach (var source in group.Sources)
            {
                if (source.Type == LayerFieldType.Text)
                {
                    ...
                    if (!string.IsNullOrEmpty(source.Key))
                        table.AddCell(new Cell().Add(new Paragraph(group.UseDashSeparator ? "-" : ":").SetFontSize((float)group.FontSizePt)).SetBorder(iText.Layout.Borders.Border.NO_BORDER));
                    else
                ...
                else if (source.Type == LayerFieldType.Image && !string.IsNullOrEmpty(source.Value))
                {
                    ...
                    var img = new Image(imgData)
                        .ScaleToFit(widthPt, heightPt)
                        .SetFixedPosition(xPt, currentY - heightPt);
