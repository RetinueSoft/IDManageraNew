...
public class PointsService(IDManagerDbContext db)
{
    ...
    /// else's is reported as not found. Only completed transactions are listed; a SuperAdmin can ask
    /// for the pending and failed ones as well (IncludeIncompleteAlso) - for anyone else the request
    /// is ignored and they still get completed ones only.
        ...
        int requestedById, GetPointsRequest request, CancellationToken ct)
    {
        ...
        var includeIncomplete = request.IncludeIncompleteAlso && requester.Role == UserRole.SuperAdmin;
        var query = db.PointTransactions
            .Where(t => t.UserId == request.UserId && (includeIncomplete || t.Status == PointStatus.Completed))
    ...
    /// The words a card's points history rows use for the card: "<template> <background> generated
    /// for <what it was generated for>" - [cardLabel] is "<template name> <background name>".
    private static string SpendReason(string cardLabel, string forName) => $"{cardLabel} generated for {forName}";
    private static string EarnReason(string cardLabel, string byUserName, string forName) => $"{cardLabel} generated by {byUserName} for {forName}";

    /// Renames the card in its points history rows (the spend and the matching earn), e.g. once
    /// the final downloaded file name (and background) is known.
    public async Task RenameCardTransactionsAsync(int idCardId, string cardLabel, string forName, CancellationToken ct)
    {
        ...
        foreach (var row in rows)
        {
            ...
            {
                row.Reason = SpendReason(cardLabel, forName);
            }
            else if (row.Type == PointTransType.EarnForCard)
            {
                var by = await db.Users.FindAsync([row.ByUserId], ct);
                row.Reason = EarnReason(cardLabel, by?.Name ?? "a member", forName);
            }
        }
        await db.SaveChangesAsync(ct);
    }

    public async Task<int> CreateIdCardAsync(IDCardEntity idCard, string cardLabel, string forName, CancellationToken ct)
    {
        ...
        db.PointTransactions.Add(new PointTransactionEntity
        {
            ...
            Reason = SpendReason(cardLabel, forName),
        ...
        db.PointTransactions.Add(new PointTransactionEntity
        {
            ...
            UserId = adminId,
            Points = idCard.PointsDeducted,
            Reason = EarnReason(cardLabel, user.Name, forName),
