...
public class CardServiceTests
{
    ...
    public async Task TheEarnRow_KeepsWhoGeneratedTheCard()
    {
        ...
        Assert.Contains("Card Default generated for Ravi Kumar", reasons);
        Assert.Contains("Card Default generated by Priya for Ravi Kumar", reasons);
    }

    // ---- "<template> <background> generated for <file>" ----

    [Fact]
    public async Task TheHistoryNamesTheTemplateAndTheBackgroundTheCardWasMadeOn()
    {
        using var testDb = TestDb.Create();
        var db = testDb.Context;
        var (templateId, defaultCombinationId) = await CreateTemplateWithCombinationAsync(db);
        var blue = (await new TemplateService(db).AddCombinationAsync(new AddCombinationCommand
        {
            TemplateId = templateId, Name = "Blue", FrontImageBytes = Png, BackImageBytes = Png,
        }, CancellationToken.None)).Value!;
        var sa = await TestUsers.AddAsync(db, "SA", UserRole.SuperAdmin);
        db.CardTemplates.Single().FileNamePattern = "{Name}";
        await db.SaveChangesAsync();
        ...
        // On the template's own images ...
        await service.GenerateAsync(sa.Id, new GenerateCardCommand { TemplateId = templateId, CombinationId = 0, PdfBytes = MemberPdf() }, CancellationToken.None);
        // ... and on the "Blue" background.
        await service.GenerateAsync(sa.Id, new GenerateCardCommand { TemplateId = templateId, CombinationId = blue.Id, PdfBytes = MemberPdf() }, CancellationToken.None);

        var spends = db.PointTransactions.Where(t => t.Type == PointTransType.SpendForCard).OrderBy(t => t.Id).Select(t => t.Reason).ToList();
        Assert.Equal(["Card Default generated for Ravi Kumar", "Card Blue generated for Ravi Kumar"], spends);
        _ = defaultCombinationId;
    }

    [Fact]
    public async Task DownloadingOnAnotherBackground_RenamesTheHistoryToThatBackground()
    {
        using var testDb = TestDb.Create();
        var db = testDb.Context;
        var (templateId, combinationId) = await CreateTemplateWithCombinationAsync(db);
        var festival = (await new TemplateService(db).AddCombinationAsync(new AddCombinationCommand
        {
            TemplateId = templateId, Name = "Festival", FrontImageBytes = Png, BackImageBytes = Png,
        }, CancellationToken.None)).Value!;
        var sa = await TestUsers.AddAsync(db, "SA", UserRole.SuperAdmin);
        db.CardTemplates.Single().FileNamePattern = "{Name}";
        await db.SaveChangesAsync();
        var service = NewService(db);
        var generated = await service.GenerateAsync(sa.Id, new GenerateCardCommand { TemplateId = templateId, CombinationId = combinationId, PdfBytes = MemberPdf() }, CancellationToken.None);

        await service.DownloadAsync(sa.Id, generated.Value!.IdCardId, null, festival.Id, CancellationToken.None);

        Assert.All(Reasons(db), reason => Assert.StartsWith("Card Festival generated", reason));
    }
}
