...
extension LayerGroupText on LayerGroup {
  ...
  /// The rows of an aligned layer: one per field that has a value, plus one blank line
  /// per "empty line" entry (trailing blank lines are dropped).
  List<BulletRow> get bulletRows {
    ...
    for (final s in sources) {
      ...
      rows.add(BulletRow((s.key ?? '').trim(), value));
    }
  ...
  /// The text of a combined layer: each field as "key: value" (just the value when
  /// its key is empty), skipping fields with no value. Each field is followed by its
  ...
  String get combinedText {
    ...
    for (final s in sources) {
      ...
      buffer.write(key.isEmpty ? value : '$key$keyValueSeparator $value');
